...
class rec():
  def __init__(self):
    ...
    self.angles_s = np.linspace(0, np.pi*0.5, num=360, endpoint=False)
    self.angles_view = np.linspace(0, np.pi, num=10, endpoint=False)
    #有限角度指的是np.pi是有限的个数 90个就是正弦图取一半
